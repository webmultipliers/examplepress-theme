...
/**
 * Should technical fallback details be shown to the current viewer?
 * Admins always; everyone else only when WP_DEBUG is on.
 */
$ep_show_tech = ( function_exists( 'current_user_can' ) && current_user_can( 'manage_options' ) )
	|| ( defined( 'WP_DEBUG' ) && WP_DEBUG );

// ── Hard dependency: Blockstudio ─────────────────────────────────
if ( ! function_exists( 'bs_block' ) ) {
	...
	<div useBlockProps style="max-width:640px;margin:4rem auto;padding:2rem;font-family:system-ui,sans-serif;background:#1a1a1e;color:#e8e8ed;border:1px solid #ff6b6b33;border-radius:8px;">
		<?php if ( $ep_show_tech ) : ?>
			<p style="margin:0 0 0.5rem"><strong style="color:#ff6b6b">Blockstudio Missing</strong></p>
			<p style="margin:0;color:#a0a0b2;font-size:0.9rem;line-height:1.6">The <code style="font-family:monospace;font-size:0.85em;background:#222;padding:0.15em 0.4em;border-radius:4px">blockstudio</code> plugin is required by this theme. Activate it to render templates.</p>
		<?php else : ?>
			<p style="margin:0;color:#a0a0b2;font-size:0.95rem;line-height:1.6">This site is being configured. Please check back shortly.</p>
		<?php endif; ?>
	</div>
	<?php
	return;
...
// ── Kernel API contract check ────────────────────────────────────
$ep_min_api    = defined( 'EP_THEME_MIN_KERNEL_API' ) ? (int) EP_THEME_MIN_KERNEL_API : 1;
$ep_kernel_api = defined( Router::class . '::API_VERSION' ) ? Router::API_VERSION : 0;
if ( $ep_kernel_api < $ep_min_api ) {
	status_header( 503 );
	nocache_headers();
	?>
	<div useBlockProps style="max-width:640px;margin:4rem auto;padding:2rem;font-family:system-ui,sans-serif;background:#1a1a1e;color:#e8e8ed;border:1px solid #ff6b6b33;border-radius:8px;">
		<?php if ( $ep_show_tech ) : ?>
			<p style="margin:0 0 0.5rem"><strong style="color:#ff6b6b">Kernel API Mismatch</strong></p>
			<p style="margin:0;color:#a0a0b2;font-size:0.9rem;line-height:1.6">Theme requires kernel API <code><?php echo (int) $ep_min_api; ?></code> (found <code><?php echo (int) $ep_kernel_api; ?></code>). Update <code>examplepress-mu</code>.</p>
		<?php else : ?>
			<p style="margin:0;color:#a0a0b2;font-size:0.95rem;line-height:1.6">This site is being configured. Please check back shortly.</p>
		<?php endif; ?>
	</div>
...
<?php else : ?>
	...
	<div useBlockProps class="ep-missing-template" style="max-width:640px;margin:4rem auto;padding:2rem;font-family:system-ui,sans-serif;background:#1a1a1e;color:#e8e8ed;border:1px solid #ff6b6b33;border-radius:8px;">
		...
		<?php else : ?>
			<p style="margin:0;color:#a0a0b2;font-size:0.95rem;line-height:1.6">Page not found.</p>
		<?php endif; ?>
	</div>
<?php endif; ?>
